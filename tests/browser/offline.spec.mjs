...
test("installed Home, Tonight and Schedule survive offline",async({browser,baseURL})=>{
  ...
  const cached=await page.evaluate(async()=>{
    const names=await caches.keys(),active=names.find(name=>name.includes("7.31.0")),keys=active?await (await caches.open(active)).keys():[];
  ...
  expect((await page.evaluate(()=>caches.keys())).filter(name=>name.startsWith("nhl-tracker-"))).toEqual(["nhl-tracker-7.31.0"]);
