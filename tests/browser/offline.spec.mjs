...
test("installed Home, Tonight and Schedule survive offline",async({browser,baseURL})=>{
  ...
  const cached=await page.evaluate(async()=>{
    const names=await caches.keys(),active=names.find(name=>name.includes("7.43.0")),keys=active?await (await caches.open(active)).keys():[];
  ...
  expect((await page.evaluate(()=>caches.keys())).filter(name=>name.startsWith("nhl-tracker-"))).toEqual(["nhl-tracker-7.43.0"]);
...
test("a previously loaded historical Player Compare direct URL survives offline",async({browser,baseURL})=>{
  ...
  expect(runtimeCache.some(path=>path.startsWith("/data/seasons/20252026-evidence.json"))).toBe(true);
  expect(runtimeCache.some(path=>path.startsWith("/data/seasons/20252026-players-CAR.json"))).toBe(true);
  expect(runtimeCache.some(path=>path.startsWith("/data/seasons/20252026.json"))).toBe(false);
