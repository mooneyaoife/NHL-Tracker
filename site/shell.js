...
(()=>{
  ...
  const directPage=new URLSearchParams(location.search).has("game")?"games":location.hash.slice(1)||"dashboard";
  if(directPage==="dashboard"){
    const homeController=new AbortController(),homeTimeout=setTimeout(()=>homeController.abort(),4000);
    fetch("data/home.json",{cache:"no-cache",signal:homeController.signal}).then(response=>response.ok?response.json():Promise.reject(new Error("Home unavailable"))).then(renderHome).then(settleHome).catch(()=>{
  ...
  const seasonManifestPromise=fetch("data/seasons/index.json",{cache:"no-cache"}).then(response=>response.ok?response.json():Promise.reject(new Error("Season list unavailable")));
  window.NHLTrackerSeasonManifestPromise=seasonManifestPromise;
  seasonManifestPromise.then(renderSeasonPicker).catch(()=>{
